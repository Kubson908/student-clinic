<script lang="ts" setup>
    ...
    const passwordRules = [(value: string) => {
        ...
        if (value.toLowerCase() !== value && /\d/.test(value) && value.length >= 8) return true;
    ...
    const nameRules = [(value: string) => {
        ...
        if(value[0].toLowerCase() !== value[0] && value.slice(1).toLowerCase() === value.slice(1)) return true;
        else return "Imię powinno zaczynać się od wielkiej litery";
    }, (value: string) => {
        if(!/\d/.test(value) && !/[^a-zA-Z0-9\u0118\u0119\u00F3\u00D3\u0141\u0142\u0104\u0105\u017B\u017C\u017A\u0179\u0106\u0107\u0143\u0144\u015A\u015B]/.test(value) && !/_/.test(value)) return true;
        else return "Imię może zawierać tylko litery";
    }]
    const surnameRules = [(value: string) => {
        if (value) return true;
        else return "Pole jest wymagane";
    }, (value: string) => {
        if(value[0].toLowerCase() !== value[0] && value.slice(1).toLowerCase() === value.slice(1)) return true;
    ...
    const repeatPasswordRules = [(value: string) => {
        if (value) return true;
        else return "Pole jest wymagane";
    }, (value: string) => {
    ...
    }, (value: string) => /^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+$/.test(value) || 'Nieprawidłowy e-mail']
    ...
    <v-card width="560px" location="center" elevation="5" class="rounded-lg">
        <v-card-item>
            <v-container class="d-flex justify-center align-center">
                <v-card height="64" width="64" color="#36BFF1" class="d-flex justify-center align-center">
        ...
        <v-card-text>
            <v-form @submit.prevent="submit" ref="form">
                ...
                <v-row justify="center">
                    <v-btn type="submit" color="blue-darken-2" class="mt-2 button">Zarejestruj się</v-btn>
                </v-row>
                <v-row justify="center">
                    <v-btn variant="text" size="small" class="mt-2 button">Mam już konto</v-btn>
                </v-row>
            </v-form>
        </v-card-text>
